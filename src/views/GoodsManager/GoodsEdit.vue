      ...
      <el-form ref="goodsForm" :model="goodsForm" label-width="80px" :rules="rules">
        ...
        <el-form-item label="食品规格" prop="goodsstandard">
          <el-radio-group v-model="goodsForm.goodsstandard">
            ...
            <el-radio label="单规格" @click.native="oClick">单规格</el-radio>
            <el-radio label="多规格" @click.native="mClick">多规格</el-radio>
          </el-radio-group>
        </el-form-item>
...
<script>
import { goodsedit } from "@/api/goods"; //引入商品管理接口
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs.goodsForm.validate(async valid => {
        if (valid) {
          let {id,goodsname,goodscategory,goodsfeature,imgUrl,isPromotion,goodsdesc,goodsstandard,packingexpense,goodsPrice,standardTableData} = this.goodsForm;
          let obj = {
              id,
            goodsname,
            goodscategory,
          ...
          let {code,msg} = await goodsedit(obj)
    ...
    oClick(){
        this.isShow = false;
        this.goodsForm.goodsstandard = '单规格'
        // console.log('isShow',this.isShow)
        // console.log('goodsfeature',this.goodsForm.goodsstandard)
    },
    // 多规格
    mClick(){
        this.isShow = true;
        this.goodsForm.goodsstandard = '多规格'
        // console.log('isShow',this.isShow)
        // console.log('goodsfeature',this.goodsForm.goodsstandard)
    }
  },
  mounted() {
      let {id,goodsname,goodscategory,goodsfeature,imgUrl,isPromotion,standard,goodsdesc} = JSON.parse(sessionStorage.getItem('goodslist'));
      let goodsfeatures = JSON.parse(goodsfeature)
      ...
      this.goodsForm = {
          ...
          goodsname,
          goodscategory,
          goodsfeature:goodsfeatures,
          imgUrl,
          isPromotion,
          goodsdesc
      }

      let arr = JSON.parse(standard)
    
     if(arr[0].goodsstandard === '单规格') {
        this.goodsForm.goodsstandard = '单规格';
        this.isShow = false;
        this.goodsForm.packingexpense = arr[0].packingexpense;
        this.goodsForm.goodsPrice = arr[0].goodsPrice;
     }else{
         this.goodsForm.goodsstandard = '多规格';
         this.isShow = true;
         this.goodsForm.standardTableData = arr;
     }
     
     console.log('isSHow',this.isShow)

      console.log('goodsfeature',arr)
  }
};
</script>
